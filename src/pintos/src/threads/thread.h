...
struct thread
  {
    ...
    int64_t sleep_until;                /* For timer_sleep. */
    struct list_elem allelem;           /* List element for all threads list. */

    /* Shared between thread.c and synch.c. */
    struct list_elem elem;              /* List element. */
    struct list locks;             		/* List of acquired locks. */
    struct lock *waits_for;             /* Lock waiting for acquire. */
...
#ifdef USERPROG
struct thread *thread_lookup (tid_t);
...
bool thread_priority_greater_func (const struct list_elem *a, const struct list_elem *b, void *aux UNUSED);
