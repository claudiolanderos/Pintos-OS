...
start_process(void *exec_)
{
    ...
    // Initialize supplement page table of the thread
    init_sup_page_table(&thread_current()->sup_page_table);
...
process_exit(void)
{
    ...
    // Delete the sup page table of the thread
             ...
             uint32_t read_bytes, uint32_t zero_bytes, bool writable)
{
    ...
    while (read_bytes > 0 || zero_bytes > 0)
    {
        ...
        page_entry_ptr_t new_page=NULL;
        
        new_page = create_page_entry(file, ofs, upage, read_bytes, zero_bytes, writable, FILE);        
        if ( new_page == NULL)
        {// If the page is already in page_table,there is an error.
            return false;
        }
        
        if (page_read_bytes < PGSIZE && page_zero_bytes < PGSIZE)
        {// If the data size is lower than PGSIZE,then the data is read from file.
            load_page_from_file(new_page);
        }       
