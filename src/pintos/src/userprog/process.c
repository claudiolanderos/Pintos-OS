...
tid_t
process_execute (const char *cmd_)
{
  tid_t tid;
  struct exec_info exec;
  exec.data = palloc_get_page (0);
  ...
  strlcpy (exec.data, cmd_, PGSIZE);
  argument_parse (exec.data, &exec.argc, &exec.argv);
  if (exec.argc == 0)
  {
    palloc_free_page (exec.data);
    return TID_ERROR;
  }

  exec.pwd = thread_current ()->pwd;
  if (exec.pwd == NULL)
  {
    exec.pwd = dir_open_root ();
  }
  else
  {
    exec.pwd = dir_reopen (exec.pwd);
  }

  tid = thread_create (exec.data, PRI_DEFAULT, start_process, &exec);
  if (tid == TID_ERROR)
  {
    palloc_free_page (exec.data);
    return TID_ERROR;
...
start_process (void *exec_)
{
  ...
  int i;
  size_t len;
  const struct exec_info *exec = exec_;
  ...
  success = load (exec->argv[0], &if_.eip, &if_.esp);
  ...
  thread_current ()->self = filesys_open(exec->argv[0]);
  file_deny_write (thread_current() -> self);

  for (i = exec->argc - 1; i >= 0; --i)
  {
    len = strlen (exec->argv[i]);
    if_.esp -= (len + 1) * sizeof (char);
    strlcpy (if_.esp, exec->argv[i], len + 1);
    exec->argv[i] = if_.esp;
  }
  ...
  *((char **) if_.esp) = NULL;
  ...
  for (i = exec->argc; i >= 0; --i)
  {
    if_.esp -= sizeof (char *);
    *((char **) if_.esp) = exec->argv[i];
  }
  ...
  *((size_t *) if_.esp) = exec->argc;

  if_.esp -= sizeof (void *);
  *((void **) if_.esp) = NULL;

  thread_set_pwd (thread_current (), exec->pwd);
  strlcpy (thread_current ()->name, exec->argv[0], sizeof (thread_current ()->name));
  palloc_free_page (exec->data);
...
process_exit (void)
{
  ...
  fd_process_exit (thread_current ());
}
...
/* Loads a segment starting at offset OFS in FILE at address
 ...
 l  user process if WRITABLE is true, read-only otherwise.
