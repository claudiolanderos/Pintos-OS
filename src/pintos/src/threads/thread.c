...
bool thread_priority_less_func (const struct list_elem *a,
                                       const struct list_elem *b,
                                       void *aux UNUSED);
bool thread_priority_greater_func (const struct list_elem *a,
                                       const struct list_elem *b,
                                       void *aux UNUSED);
...
thread_tick (void) 
{
  ...
  {
    e = list_min (&sleep_list, thread_sleep_less_func, NULL);
  ...
  if (thread_mlfqs)
  {
    ...
    {
      thread_current ()->recent_cpu = real_add (thread_current ()->recent_cpu,
                                                real_of_int (1));
    }

    if (ticks % TIMER_FREQ == 0)
    {
      int ready_thread = list_size (&ready_list) + (thread_current () == idle_thread ? 0 : 1);
      load_avg = real_div (real_add (real_mult (real_of_int (59),
                                                load_avg),
                                     real_of_int (ready_thread)),
                           real_of_int (60));
                        ...
                        void *aux UNUSED) 
{
  ...
  ta = (const struct thread *) list_entry (a, const struct thread, elem);
  tb = (const struct thread *) list_entry (b, const struct thread, elem);
  return (ta->sleep_until < tb->sleep_until) ||
      (ta->sleep_until == tb->sleep_until &&
...
bool
thread_priority_greater_func (const struct list_elem *a,
                           const struct list_elem *b,
                           void *aux UNUSED)
{
  const struct thread *ta, *tb;
...
/* Sets the current thread's priority to NEW_PRIORITY. This also considers about lock. */
void
thread_set_priority (int new_priority) 
{
  ...
  {
		t = list_entry (e, struct lock, elem);
    if (new_priority < t->highest_priority) new_priority = t->highest_priority;
  }
...
calculate_priority (struct thread *t,
                    void *aux UNUSED)
{
  t->priority = PRI_MAX - int_of_real (real_div (t->recent_cpu,
                                                 real_of_int (4))) - (t->nice * 2);
...
{
  return int_of_real (real_mult (load_avg,
                                 real_of_int (100)));
}
...
{
  return int_of_real (real_mult (thread_current ()->recent_cpu,
                                 real_of_int (100)));
}
...
calculate_recent_cpu (struct thread *t,
                      void *aux UNUSED)
{
  t->recent_cpu = real_add (real_mult (t->recent_cpu,
                                       real_div (real_mult (load_avg,
                                                            real_of_int (2)),
                                                 real_add (real_mult (load_avg,
                                                                      real_of_int (2)),
                                                           real_of_int (1)))),
                            real_of_int (t->nice));
}
